...
## Project Overview
This project analyzes customer churn behavior using the **Telco Customer Churn dataset** from [Kaggle](https://www.kaggle.com/datasets/blastchar/telco-customer-churn).  
The goal is to identify **key drivers of churn** and build a predictive model that can flag at-risk customers with strong accuracy and interpretability.
...
- This imbalance informed the decision to apply SMOTE during model training.

### Key Drivers of Churn
| Feature | Observation | Insight |
|----------|--------------|----------|
| Contract Type | 42.7% churn for month-to-month vs 2.8% for 2-year | Long-term contracts dramatically reduce churn. |
| Internet Service | 41.9% churn for Fiber optic vs 19% for DSL | Fiber customers may be more price-sensitive. |
| Payment Method | Highest churn among Electronic check users | Manual payment users churn more often. |
| Online Security / Tech Support | Customers without these add-ons churned 2–3× more | Bundled services improve retention. |
| Paperless Billing | Appears correlated with higher churn, but driven by overlap with month-to-month and electronic check customers | Correlation, not causation. |
...
The best model achieved **ROC-AUC ≈ 0.84** on the test set with improved precision–recall balance.
...
## Conclusion
This project successfully demonstrates a full end-to-end churn prediction workflow:
- Exploratory analysis to uncover churn drivers  
- Data balancing with SMOTE  
- Model tuning for performance optimization  
- Threshold adjustment for business relevance  
- Feature interpretation for actionable retention strategies  

The tuned **XGBoost model (ROC-AUC ≈ 0.84)** provides a strong predictive foundation for proactive churn management.
